...
//  AngryBirdClone
//
//  Created by Burak Afyonlu on 17.02.2023.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
       ...
       let texture2 = SKTexture(imageNamed: "box")
        box = SKSpriteNode(texture: texture2)
        box.position = CGPoint(x: 1, y: 1)
        box.size = CGSize(width: self.frame.width / 15, height: self.frame.height / 15)
        box.zPosition = 2
        self.addChild(box)*/
        ...
        bird.physicsBody?.node?.zPosition = 2
